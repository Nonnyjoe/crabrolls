...
impl Supervisor {
    ...
    async fn handle_advance_input(
        ...
        if rollup.get_app_address().await.is_none() {
            rollup.set_app_address(app_address).await;
            debug!(
                "Application contract address received and stored as {}",
                app_address
            );
        }
        ...
        if let PortalHandlerConfig::Handle { .. } = options.portal_config {
            ...
            .get_address_book()
            .is_portal(advance_input.metadata.sender)
