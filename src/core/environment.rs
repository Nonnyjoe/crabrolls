...
impl EtherEnvironment for Rollup {
    ...
    async fn ether_withdraw(&self, address: Address, value: Uint) -> Result<(), Box<dyn Error>> {
        let app_address: Option<ethabi::ethereum_types::H160> = self.get_app_address().await;
        ...
        self.send_voucher(app_address.expect("App address is not set"), payload, 0)
            .await?;
