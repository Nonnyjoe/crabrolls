use crate::{
    core::contracts::{
        erc1155::ERC1155Wallet, erc20::ERC20Wallet, erc721::ERC721Wallet, ether::EtherWallet,
    },
    ...
    fn try_from(deposit: Deposit) -> Result<Self, Self::Error> {
        match deposit {
            ...
            Deposit::ERC20 {
                ...
                token,
                amount,
            } => Ok(ERC20Wallet::deposit_payload(sender, token, amount)?),
            Deposit::ERC721 { sender, token, id } => {
                Ok(ERC721Wallet::deposit_payload(sender, token, id)?)
            }
            Deposit::ERC1155 {
                sender,
                token,
